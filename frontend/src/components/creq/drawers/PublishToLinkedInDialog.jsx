...
export default function PublishToLinkedInDialog({ proj, onClose, busy, progress, onPublish }) {
  ...
  return (
    <div className="fixed inset-0 bg-ink/40 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl w-full max-w-lg max-h-[85vh] overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()} data-testid="publish-linkedin-dialog">
        <div className="flex items-center gap-3 px-6 py-4 border-b border-line">
          ...
          <div className="text-caption text-neutral-500 ml-2">Creates a draft in Social EQ, then lands in the approval queue.</div>
        ...
        <div className="px-6 py-4 space-y-4 flex-1 overflow-y-auto">
          ...
          <label className="block">
            <span className="ui-label">Topic — used to generate the caption & hashtags</span>
            <input type="text" value={topic} onChange={(e) => setTopic(e.target.value)} data-testid="publish-topic"
              placeholder={proj?.topic || "e.g. How we built our sales pipeline"} className="w-full input" />
          </label>
        </div>
        ...
        <div className="px-6 py-4 border-t border-line flex items-center justify-between gap-3 flex-shrink-0">
          ...
          ) : <span className="text-caption text-neutral-500">Rendering stays local — nothing is posted until you approve in the queue.</span>}
