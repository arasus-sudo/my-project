...
const STATUS_COLOR = {
  registered: "text-neutral-400 border-neutral-300",
  ...
  no_answer: "text-neutral-400 border-neutral-300",
  busy: "text-neutral-400 border-neutral-300",
};

const SENTIMENT_COLOR = { positive: "text-green-700", neutral: "text-neutral-400", negative: "text-red-600" };
      ...
      <div className="animate-fade-in px-6 sm:px-8">
        {loading ? <div className="text-neutral-400 text-sm">Loading…</div> : calls.length === 0 ? (
          <div className="shadow-card rounded-2xl p-10 text-center">
            <div className="font-display text-xl sm:text-2xl font-semibold">No calls yet</div>
            <p className="text-sm text-neutral-400 mt-2">Click-to-call a lead or launch a voice campaign to see logs here.</p>
          </div>
        ) : (
          ...
          <div className="bg-white border-l border-line h-full w-full max-w-lg overflow-y-auto p-4 sm:p-6 space-y-4">
            <div className="flex items-start justify-between">
              <div>
                ...
                <div className="text-xs text-neutral-400 font-mono flex items-center gap-2 mt-1">
                  {detail.to_number} · {detail.status} <VoiceProviderBadge provider={detail.provider} />
                </div>
              </div>
                ...
                <div className="space-y-1">
                  {Object.entries(detail.qualification).map(([k, v]) => (
                    <div key={k} className="text-sm flex gap-2">
                      <span className="font-mono text-xs text-neutral-400 min-w-[100px]">{k}</span>
                      <span>{String(v)}</span>
                    </div>
                  ))}
                </div>
            ...
            {!detail.summary && !detail.transcript && !detail.recording_url && (
              <p className="text-sm text-neutral-400">Call is still in progress or hasn't been analyzed yet.</p>
            )}
          </div>
        </div>
      )}
