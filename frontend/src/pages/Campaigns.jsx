      ...
      <div className="p-6">
        {loading ? <div className="text-neutral-500 text-sm">Loading…</div> : items.length === 0 ? (
          <div className="card-flat p-10 text-center">
            <div className="font-display text-xl font-bold">No campaigns yet</div>
            <p className="text-sm text-neutral-500 mt-2">Create your first sequence to start booking meetings.</p>
                ...
                {items.map((c) => (
                  <tr key={c.id} className="border-b border-line hover:bg-surfacehover">
                    <td className="p-4">
                      ...
                      <div className="text-xs text-neutral-500 font-mono">{c.steps?.length || 0} steps · {c.lead_ids?.length || 0} leads</div>
                    </td>
                    <td className="p-4">
