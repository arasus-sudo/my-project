...
export default function PostQueue() {
  ...
  return (
    <div>
      ...
      <div className="p-6">
        {loading ? <div className="text-neutral-500 text-sm">Loading…</div> : posts.length === 0 ? (
          <div className="card-flat p-10 text-center text-sm text-neutral-500">No posts yet.</div>
        ) : (
      ...
      {detail && (
        <div className="fixed inset-0 bg-ink/40 flex items-center justify-center z-50" onClick={(e) => e.target === e.currentTarget && setDetail(null)}>
          <div className="bg-white border border-line p-6 rounded-sm w-full max-w-lg space-y-3">
            <div className="flex items-start justify-between">
              <div>
                <div className="ui-label text-neutral-500 capitalize">{detail.platform}</div>
                <div className="font-display font-bold text-lg">{detail.headline}</div>
              </div>
