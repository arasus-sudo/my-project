...
export default function Proposals() {
  ...
  return (
    <div>
      <PageHeader
        title="Proposals"
        subtitle="AI-researched proposals generated from your CRM, exportable to PDF or PowerPoint."
      ...
      <div className="p-6">
        {loading ? <div className="text-neutral-500 text-sm">Loading…</div> : items.length === 0 ? (
          <div className="card-flat p-10 text-center">
            <div className="font-display text-xl font-bold">No proposals yet</div>
            <p className="text-sm text-neutral-500 mt-2">Pick a lead and Proposal EQ will research and draft a deck.</p>
              ...
              <tbody>
                {items.map((p) => (
                  <tr key={p.id} className="border-b border-line hover:bg-surfacehover">
                    ...
                    <td className="p-3 text-neutral-600">{p.lead ? `${p.lead.first_name} ${p.lead.last_name || ""} · ${p.lead.company || ""}` : "—"}</td>
