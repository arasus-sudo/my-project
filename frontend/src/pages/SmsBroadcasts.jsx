  ...
  return (
    <div>
      <PageHeader title="SMS Broadcasts" subtitle="Send bulk SMS to your contacts."
        right={<Button variant="primary" icon={Plus} onClick={() => setModal(true)}>New broadcast</Button>}
      />
      <div className="animate-fade-in px-6 sm:px-8 py-6 space-y-3">
        {items.length === 0 ? (
          <EmptyState title="No broadcasts yet" description="Create a broadcast to send bulk SMS to your contacts."
            actionLabel="New broadcast" onAction={() => setModal(true)} />
        ) : items.map((b) => (
          <Card key={b.id}>
      ...
      <Modal open={modal} onOpenChange={setModal}>
        <ModalContent size="sm" title="New SMS Broadcast"
          footer={
            <>
              <Button variant="secondary" onClick={() => setModal(false)}>Cancel</Button>
              <Button variant="primary" type="submit" form="sms-broadcast-form">Create</Button>
            </>
          }
        >
          <form id="sms-broadcast-form" onSubmit={save} className="space-y-3">
