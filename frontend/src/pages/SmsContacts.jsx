...
export default function SmsContacts() {
  ...
  const load = () => api.get("/sms-eq/contacts").then((r) => setItems(r.data.items || []));
  useEffect(() => { load(); }, []);

  const save = async (e) => {
    e.preventDefault();
    try {
      await api.post("/sms-eq/contacts", { ...form, tags: form.tags.split(",").map(t => t.trim()).filter(Boolean) });
  ...
  const toggleOptOut = async (id, optedOut) => {
    try {
      await api.put(`/sms-eq/contacts/${id}`, { opted_out: !optedOut });
