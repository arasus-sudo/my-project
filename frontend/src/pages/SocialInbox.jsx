...
const STATUS_META = {
  ...
  ignored: { t: "Ignored", c: "text-neutral-400 border-neutral-400" },
};
          ...
          {filtered.map((c) => (
            <button key={c.id} onClick={() => select(c)} data-testid={`comment-${c.id}`}
              ...
              <div className="flex items-center justify-between">
                <div className="font-medium truncate">{c.author || "Someone"}</div>
                <span className={`ui-label border px-1.5 py-0.5 text-[9px] ${STATUS_META[c.status]?.c || STATUS_META.new.c}`}>
                  {STATUS_META[c.status]?.t || "New"}
                </span>
              </div>
              <div className="text-xs text-neutral-400 truncate mt-1 capitalize">
                {PLATFORM_LABEL[c.platform] || c.platform} · {c.post_headline || "post"}
              </div>
              <div className="text-xs text-neutral-700 mt-2 line-clamp-2">{c.text}</div>
            </button>
          ))}
        </div>
          ...
          {active ? (
            <>
              <div className="p-4 border-b border-line bg-white">
                <div className="font-display font-semibold">{active.author || "Someone"}</div>
                <div className="text-xs text-neutral-400 font-mono capitalize">{PLATFORM_LABEL[active.platform] || active.platform}</div>
              </div>
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                <div className="max-w-md">
                  <div className="ui-label mb-1">{active.author || "Someone"}</div>
                  <div className="p-3 text-sm rounded-xl border bg-white border-line">{active.text}</div>
                </div>
                {active.replied_text && (
                  <div className="max-w-md ml-auto">
                    <div className="ui-label mb-1">You</div>
                    <div className="p-3 text-sm rounded-xl border bg-brand-gradient text-white border-transparent">{active.replied_text}</div>
                  </div>
                )}
              </div>
                ...
                <div className="p-4 border-t border-line bg-white space-y-2">
                  {active.ai_suggested_reply && reply === active.ai_suggested_reply && (
                    <div className="flex items-center gap-1.5 text-[11px] text-accent">
                      <Sparkles size={11} /> AI-suggested reply — edit before sending
                    </div>
                  )}
            ...
            <div className="p-8 text-neutral-400 text-sm">Select a comment</div>
          )}
        </div>
        ...
        <aside className="hidden lg:block col-span-2 border-l border-line bg-white p-4 overflow-y-auto">
          {active?.post ? (
            <>
              ...
              <div className="font-display font-semibold mt-1 text-sm">{active.post.headline}</div>
              <p className="text-xs text-neutral-400 mt-2 line-clamp-4">{active.post.body}</p>
              {active.post.platform_post_url && (
                <a href={active.post.platform_post_url} target="_blank" rel="noreferrer"
                  className="text-xs text-accent hover:underline mt-3 inline-block">View live post</a>
              )}
            </>
          ) : (
