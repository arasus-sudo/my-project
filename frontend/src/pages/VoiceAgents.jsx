...
export default function VoiceAgents() {
  ...
  const sync = async (id) => {
    setSyncing(id);
    try { await api.post(`/voice-eq/agents/${id}/sync`); toast.success("Synced to Retell"); load(); }
      ...
      <div className="p-6">
        {loading ? <div className="text-neutral-500 text-sm">Loading…</div> : items.length === 0 ? (
          <div className="card-flat p-10 text-center">
            <div className="font-display text-xl font-bold">No voice agents yet</div>
            <p className="text-sm text-neutral-500 mt-2">Create a persona to start calling leads.</p>
                ...
                {items.map((a) => (
                  <tr key={a.id} className="border-b border-line hover:bg-surfacehover">
                    <td className="p-4">
                      ...
                      <div className="text-xs text-neutral-500 font-mono">v{a.version} · {a.language}</div>
                    </td>
                    <td className="p-4 text-neutral-600 capitalize">{a.purpose}</td>
                    <td className="p-4 font-mono text-xs text-neutral-600">{a.voice_id}</td>
                    <td className="p-4"><StatusBadge status={a.status} /></td>
                    <td className="p-4 text-right space-x-2 whitespace-nowrap">
                      <button data-testid={`sync-${a.id}`} onClick={() => sync(a.id)} disabled={syncing === a.id}
