      ...
      <div className="animate-fade-in px-6 sm:px-8">
        {loading ? <div className="text-neutral-400 text-sm">Loading…</div> : items.length === 0 ? (
          <div className="shadow-card rounded-2xl p-10 text-center">
            <div className="font-display text-xl sm:text-2xl font-semibold">No voice campaigns yet</div>
            <p className="text-sm text-neutral-400 mt-2">Pick an agent and a lead list to start dialing.</p>
                ...
                {items.map((c) => (
                  <tr key={c.id} className="border-b border-line hover:bg-surfacehover">
                    <td className="p-4">
                      ...
                      <div className="text-xs text-neutral-400 font-mono">{c.lead_ids?.length || 0} leads · {c.send_window_start}–{c.send_window_end} {c.timezone}</div>
                    </td>
